...
using PowerBuildToolDevOpsAPI.Helper;
...
namespace PowerBuildToolDevOpsAPI.DevOpsBuilds.Manager.BuildManager
{
    public class BuildManager
    {
        ...
        /// <returns></returns>
        ///        
        public async Task<int> CreateCRMTestBuild(string solname)
        {
            int buildId = 0;
            try
            {
                // var devopsPool = _builderPool.Get();//get object
                ...
                /* using (var buildclient = new BuildHttpClient(new Uri(devopsPool.CollUrl), devopsPool.VssCredentials))
                 {

                     var definitions = await buildclient.GetDefinitionsAsync(c_projectName);
                     // Get the specified name of build definition.
                     var target = definitions.First(d => d.Name == "CRMTestSolDeployment");
                                            ...
                                            int logid, BuildHttpClient client)
        {
            ...
            try
            {
                ...
                if (logContents.Count > 0)
                    resultModel = await GetformatedHtmlContents(logContents);
            }
            catch (Exception)
            {
            }
        ...
        async Task<ResultModel> GetformatedHtmlContents(List<string> logContents)
        {
            ResultModel resultModel = new ResultModel();
